...
var periodTime = performance.now();
        ...
        if (GUI.connect_lock != true) { // GUI control overrides the user control
            ...
            if (selected_port != '0' && selected_port != 'DFU') {
                if (!clicks) {
                    ...
                    $('#fault').css("background-color","#444444").text('');
...
function read_serial(info) {
    ...
    if (periodTick == 16){
        ...
        var dif = Math.floor((nowTime - periodTime)/16);
        periodTime = nowTime;
        
        $('span.statusbar_update_period').text(chrome.i18n.getMessage('statusbar_update_period', [dif]));
